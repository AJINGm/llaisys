...
namespace llaisys::ops {
    void embedding(tensor_t out, tensor_t index, tensor_t weight) {
        ...
        ASSERT(out->shape()[0] == index->shape()[0],
               "Embedding: output tensor's first dimension must match index tensor's size.");
        ASSERT(out->shape()[1] == weight->shape()[1],
               "Embedding: output tensor's second dimension must match weight tensor's second dimension.");
        ASSERT(weight->isContiguous() && out->isContiguous() && index->isContiguous(),
               "Embedding: weight index and weight tensors must be contiguous.");

        // always support cpu calculation
        if (out->deviceType() == LLAISYS_DEVICE_CPU) {
            return cpu::embedding(out->data(), index->data(), weight->data(), out->dtype(), index->shape()[0],
